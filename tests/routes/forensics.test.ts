...
import {
  createForensicsRouter,
  type ForensicsBlockscout,
} from '../../src/routes/forensics.js';
import type {
  BlockscoutAddress,
  BlockscoutHolders,
  BlockscoutToken,
...
function makeApp(mock: ForensicsBlockscout): express.Express {
  ...
  app.use('/api/v1/x402/base', createForensicsRouter({ blockscout: mock }));
  return app;
}
...
describe('GET /token/:address/forensics', () => {
  it('rejects invalid addresses with 400', async () => {
    const app = makeApp(mockBlockscout({}));
    const res = await request(app).get('/api/v1/x402/base/token/0xNOPE/forensics');
  ...
  it('happy path: computes all fields with deployer enrichment', async () => {
    ...
    const res = await request(makeApp(mock)).get(
      `/api/v1/x402/base/token/${TOKEN}/forensics`,
    );
  ...
  it('lp_locked_heuristic true when ?pair has the dead address in top-5 LP holders', async () => {
    ...
    const res = await request(makeApp(mock)).get(
      `/api/v1/x402/base/token/${TOKEN}/forensics?pair=${PAIR}`,
    );
    expect(res.status).toBe(200);
